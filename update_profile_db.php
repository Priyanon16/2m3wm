...
if (isset($_POST['Submit'])) {
    ...
    // 3. ตรวจสอบว่าเคยมีข้อมูลในตาราง address หรือยัง
    ...
    if (mysqli_num_rows($check_addr) > 0) {
        ...
        $sql_addr = "UPDATE address SET phone = '$phone', address = '$address' WHERE user_id = '$uid'";
    } else {
        // ยังไม่มีให้ INSERT
        $sql_addr = "INSERT INTO address (user_id, phone, address) VALUES ('$uid', '$phone', '$address')";
    }
    
    if (mysqli_query($conn, $sql_addr)) {
        echo "<script>alert('บันทึกข้อมูลสำเร็จ'); window.location='setting.php';</script>";
    } else {
